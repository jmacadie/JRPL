...
if (!$result) {
  $error =
    'Error fetching tournament roles: <br />' .
    mysqli_error($link) .
    '<br /><br />' .
    $sql;
...
while($row = mysqli_fetch_assoc($result)) {
  ...
  if ($row['TournamentRole'] != $tr) {
    ...
    // If $tr is set then must have already done a loop so push completed TR
    // array onto output array
    if ($tr != '') {
  ...
  $arrTRTOut[] = array('id' => $row['SelectTeamID'],
                       'name' => $row['SelectTeam']);
}
